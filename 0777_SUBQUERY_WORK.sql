...
-- 메인: 1) 직급이 대리인 직원들의 사번, 이름, 직급 ,급여 조회 (메인)
SELECT e.EMP_ID , e.EMP_NAME, JOB_NAME, e.SALARY  FROM EMPLOYEE e JOIN JOB USING (JOB_CODE) WHERE JOB_NAME ='대리';


-- 서브: 2) 직급이 과장인 직원들의 급여 조회 (서브)
...
WHERE LOCATION_ID = (SELECT l.LOCAL_CODE  FROM LOCATION l WHERE l.NATIONAL_CODE ='KO');

-- 3) EMPLOYEE테이블에서 위 결과들과 동일한 DEPT_CODE를 가진 사원을 조회한다


SELECT EMP_NAME, DEPT_CODE FROM EMPLOYEE 
WHERE DEPT_CODE IN (  SELECT d.DEPT_ID  FROM DEPARTMENT d 
WHERE LOCATION_ID = (SELECT l.LOCAL_CODE  FROM LOCATION l WHERE l.NATIONAL_CODE ='KO')    ) ;
...
FROM EMPLOYEE e JOIN JOB USING (JOB_CODE) LEFT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID) 
